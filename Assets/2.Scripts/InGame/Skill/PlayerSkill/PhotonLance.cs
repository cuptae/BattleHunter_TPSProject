...
public class PhotonLance : ActiveSkill
{
    ...
    public override IEnumerator Activation()
    {
        ...
        {
            GameObject rightEffect = PoolManager.Instance.GetObject(activeData.skillName + "Vfx"+GameManager.Instance.userId, firePos.position, Quaternion.LookRotation(firePos.forward));
            GameObject leftEffect = PoolManager.Instance.GetObject(activeData.skillName + "Vfx"+GameManager.Instance.userId, firePos.position, Quaternion.LookRotation(firePos.forward));
        ...
        {
            GameObject effect = PoolManager.Instance.GetObject(activeData.skillName + "Vfx"+GameManager.Instance.userId, firePos.position, Quaternion.LookRotation(firePos.forward));
            float timer = 0f;
            while (timer < duration)
            {
